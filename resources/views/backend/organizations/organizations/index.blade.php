...
@section('breadcrumbs')
    @include('backend.partials.default.breadcrumbs', ['breadcrumbs' => [
        route('backend.organizations.index') => trans('organizations.title'),
    ]])
@endsection

@section('sidebar')
    <a href="{{ route('backend.organizations.create') }}" class="btn btn-primary btn-block m-b-30">
        <i class="fa fa-plus"></i> {{ trans('global.add') }}
    </a>
    <p class="menu-title all-caps">{{ trans('organizations.index_sidebard.submenu_title') }}</p>
    <ul class="main-menu">
        {{--<li class="">--}}
            {{--<a href="{{ route('backend.organizations.export') }}">--}}
...
@section('content')
    <div class="panel panel-transparent">
        <div class="panel-heading ">
            <div class="panel-title"><i class="fa fa-code"></i> {!! trans('organizations.title') !!}</div>
        </div>
        <div class="panel-body">
            @if ($organizations['meta']['pagination']['total'])
                <div class="table-responsive">
                    <table class="table table-hover table-condensed" id="condensedTable">
                        ...
                        <tbody>
                        {{--@foreach ($organizations['data'] as $project)--}}
                            {{--<tr>--}}
                                ...
                                {{--</td>--}}
                                {{--<td class="v-align-middle semi-bold text-center">--}}
                                    {{--<a href="{{ route('backend.organizations.show', ['id' => $project['id']]) }}">{{ $project['civility_name'] }}</a>--}}
                                {{--</td>--}}
                                {{--<td class="v-align-middle semi-bold text-center">{{ $project['email'] }}</td>--}}
                                {{--<td class="v-align-middle text-center">--}}
                                    {{--<a href="{{ route('backend.organizations.edit', ['id' => $project['id']]) }}"--}}
                ...
                <div>
                    {!! trans('organizations.index_total_organizations', ['total_user' => $organizations['meta']['pagination']['total']]) !!}
                    {!! pages_pagination(
                        $organizations['meta']['pagination']['count'],
                        $organizations['meta']['pagination']['total'],
                        $organizations['meta']['pagination']['current_page'],
                        $organizations['meta']['pagination']['per_page']
                    ) !!}
                </div>
            @else
                <div class="box-body">
                    <div class="alert alert-info alert-dismissible alert-module">
                        <h4><i class="icon fa fa-info-circle"></i> {!! trans('organizations.index_no_data_title') !!}</h4>
                        {!! trans('organizations.index_no_data_description') !!}
                    </div>
                </div>
            @endif
        </div>
                        ...
                        {{--<div class="container-xs-height full-height">--}}
                            {{--<div class="row-xs-height">--}}
                                {{--<div class="modal-body col-xs-height col-middle text-center   ">--}}
                                    ...
                                    {{--<p>{{ trans('organizations.delete_message', ['username' => $project['civility_name']]) }}</p>--}}
                                    {{--<br>--}}
                                    {{--{!! Form::open(['route' => ['backend.organizations.destroy', $project['id']], 'method' => 'DELETE']) !!}--}}
