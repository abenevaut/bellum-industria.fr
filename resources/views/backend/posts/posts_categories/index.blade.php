...
@section('breadcrumbs')
    @include('backend.partials.default.breadcrumbs', ['breadcrumbs' => [
        route('backend.posts.index') => trans('posts.categories.title'),
    ]])
@endsection
...
@section('sidebar')
    ...
    <p class="menu-title all-caps">{{ trans('posts.categories.index_sidebard.submenu_title') }}</p>
    <ul class="main-menu">
        {{--<li class="">--}}
...
@section('content')
    <div class="panel panel-transparent">
        <div class="panel-heading ">
            <div class="panel-title"><i class="fa fa-list"></i> {!! trans('posts.categories.title') !!}</div>
        </div>
        <div class="panel-body">
            @if ($posts_categories['meta']['pagination']['total'])
                <div class="table-responsive">
                    <table class="table table-hover table-condensed" id="condensedTable">
                        ...
                        <tbody>
                        {{--@foreach ($posts_categories['data'] as $post)--}}
                            {{--<tr>--}}
                ...
                <div>
                    {!! trans('posts.categories.index_total_posts', ['total_user' => $posts_categories['meta']['pagination']['total']]) !!}
                    {!! pages_pagination(
                        $posts_categories['meta']['pagination']['count'],
                        $posts_categories['meta']['pagination']['total'],
                        $posts_categories['meta']['pagination']['current_page'],
                        $posts_categories['meta']['pagination']['per_page']
                    ) !!}
                </div>
            @else
                <div class="box-body">
                    <div class="alert alert-info alert-dismissible alert-module">
                        <h4><i class="icon fa fa-info-circle"></i> {!! trans('posts.categories.index_no_data_title') !!}</h4>
                        {!! trans('posts.categories.index_no_data_description') !!}
                    </div>
                </div>
            @endif
        </div>
                        ...
                        {{--<div class="container-xs-height full-height">--}}
                            {{--<div class="row-xs-height">--}}
                                {{--<div class="modal-body col-xs-height col-middle text-center   ">--}}
                                    ...
                                    {{--<p>{{ trans('posts.categories.delete_message', ['username' => $post['civility_name']]) }}</p>--}}
                                    {{--<br>--}}
