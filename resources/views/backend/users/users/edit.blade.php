...
@section('css')
    <link class="main-stylesheet" href="{{ mix('assets/css/backend/users/users/form.css') }}" rel="stylesheet" type="text/css"/>
@endsection

@section('js')
    <script type="text/javascript" src="{{ mix('assets/js/backend/users/users/form.js') }}"></script>
@endsection

@section('breadcrumbs')
    @include('backend.partials.default.breadcrumbs', ['breadcrumbs' => [
        route('backend.users.index') => trans('users.title'),
        route('backend.users.edit', ['id' => $user['data']['id']]) => trans('users.edit.title'),
    ]])
@endsection
            ...
            <div class="panel-body">
                <div class="row">
                    <div class="col-sm-10">
                        ...
                        {!! Form::open(['route' => ['backend.users.update', $user['data']['id']], 'id' => 'form-work', 'class' => 'form-horizontal', 'role' => 'form', 'autoprimary' => 'off', 'novalidate' => 'novalidate', 'data-user_identifier' => $user['data']['identifier'], 'method' => 'PUT']) !!}
                        ...
                        <div class="form-group required">
                            ...
                            <div class="col-sm-9">
                                <select name="role" class="full-width" data-init-plugin="select2">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-9">
                                ...
                                <a href="{{ route('backend.users.index') }}" class="btn btn-default pull-right">{{ trans('global.back') }}</a>
                            </div>
                        </div>
                        {{ Form::close() }}
                    </div>
