...
@section('breadcrumbs')
    @include('backend.partials.default.breadcrumbs', ['breadcrumbs' => [
        ...
        route('backend.users.show', ['id' => $user['data']['id']]) => trans('users.show.title', ['username' => $user['data']['full_name']]),
    ]])
@endsection

{{--@section('sidebar')--}}
    {{--<a href="{{ route('backend.users.create') }}" class="btn btn-primary btn-block btn-compose m-b-30">--}}
